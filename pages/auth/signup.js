...
export default function SignUp() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Appel à votre API pour créer un compte
      ...
      if (!res.ok) {
        ...
        if (contentType && contentType.includes('application/json')) {
          ...
          // Si ce n'est pas du JSON, ne pas essayer de lire le corps
          errorMessage = `Erreur du serveur: ${res.status}`;
        }
        throw new Error(errorMessage);
      }
  
      // Si la réponse est OK, lire le corps comme JSON
      const data = await res.json();
      
      // Redirection vers la page de connexion
      router.push('/auth/signin?success=Compte créé avec succès');
    } catch (err) {
  ...
  return (
    <div className="font-['Be_Vietnam_Pro',sans-serif] min-h-screen bg-gradient-to-b from-gray-50 to-white flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
      <Head>
        <title>Inscription | ConfessX</title>
        <meta name="description" content="Créez un compte sur ConfessX" />
            ...
            >
              {loading ? (
                ...
                <span className="absolute left-0 inset-y-0 flex items-center pl-3">
                  <svg className="h-5 w-5 text-primary/80 group-hover:text-primary/60" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
